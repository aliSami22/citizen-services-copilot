...
## Root Cause: Why the refusal gate misses 5/5 adversarial cases

- HybridFusionEngine uses RRF normalization: rank-1 in either list scores ~0.50.
- The MinRelevanceScore gate of 0.40 therefore fires only when BOTH the dense and keyword candidate lists are empty.
- Any out-of-corpus query still surfaces a rank-1 chunk, which scores >= 0.50 and is returned as an answer instead of a refusal.
- Confirmation: ADV-001..004 all measure max=0.5000 exactly (single-list rank-1); ADV-005 also clears the gate via a keyword hit.
- Mitigation (deferred to Checkpoint B): replace the absolute-score gate with a relative-margin gate (top score vs runner-up delta) AND a per-list floor. This is the change that will move refusal accuracy from 27/32 toward target.
...
1. **Refusal gate is rank-relative, not score-relative.** See the Root Cause section above for why this misses all 5 adversarial refusal cases and for the mitigation deferred to Checkpoint B.
